...
Page({
  data: {
    ...
    isPass: true, //是否通过了验证
    keyCodeBtn: '获取验证码', //显示在获取验证码按钮上的文字  18620453724
  },
  //装填手机号
  phoneInput: function (e) {
    let _that = this;
    _that.setData({  phone: e.detail.value });
  },
        ...
        success(res) {
          if (res.data.status == 20017) {
            ...
            wx.showToast({
              ...
              icon: 'success',
              success(res) {
                wx.redirectTo({ url: "../../addNew/recruiter/recruiter?key=register" });
              }
            });
          } else {
            wx.showModal({
              title: '提示信息',
              content: '接口出错!状态：' + res.data.status,
              showCancel: false
            });
          }
        },
  ...
  onLoad: function () {
    this.setData({ sessionId: wx.getStorageSync('wechat_session').sessionId });
  }
});